...
export default function ExperiencesPage() {
  ...
  const getTitle = (experience: Experience) => language === 'es' ? experience.titleEs : experience.titleFi;
  const getDescription = (experience: Experience) => language === 'es' ? experience.shortDescriptionEs : experience.shortDescriptionFi;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-aurora mx-auto mb-4" />
      ...
      <div className="bg-midnight text-snow py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-4xl font-bold">{language === 'es' ? 'Experiencias' : 'Kokemukset'}</h1>
            <button
          ...
          <p className="text-lg text-snow/80">
            {language === 'es' ? 'Descubre experiencias premium en Finlandia' : 'Tutustu Suomen premium-kokemuksiin'}
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            >
              ...
              {destinations.map((dest) => (
                <option key={dest.id} value={dest.id}>{language === 'es' ? dest.nameEs : dest.nameFi}</option>
              ))}
            </select>
            <select
            ...
            >
              ...
              {MOCK_CATEGORIES.map((cat) => (
                <option key={cat.id} value={cat.id}>{language === 'es' ? cat.nameEs : cat.nameFi}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        {filteredExperiences.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-slate text-lg">{language === 'es' ? 'No se encontraron experiencias' : 'Kokemuksia ei löytynyt'}</p>
          </div>
        ) : (
              ...
              <Link key={experience.id} href={`/experiencias/${experience.slug}`}>
                <div className="group bg-white rounded-lg overflow-hidden shadow hover:shadow-lg transition cursor-pointer h-full flex flex-col">
                  <div className="relative h-48 overflow-hidden bg-slate/10">
                    <img src={getHeroImage(experience)} alt={getTitle(experience)} className="w-full h-full object-cover group-hover:scale-110 transition duration-300" />
                    <div className="absolute top-4 right-4 bg-aurora text-midnight px-3 py-1 rounded-full text-sm font-semibold">{getCategoryName(experience.categoryId)}</div>
                  </div>
                  <div className="p-4 flex-1 flex flex-col">
                    <h3 className="text-lg font-bold text-midnight mb-2 line-clamp-2">{getTitle(experience)}</h3>
                    <p className="text-slate text-sm mb-4 line-clamp-2 flex-1">{getDescription(experience)}</p>
                    <div className="space-y-2 text-sm text-slate mb-4">
                      <div className="flex items-center gap-2"><MapPin className="w-4 h-4" /><span>{getDestinationName(experience.destinationId)}</span></div>
                      <div className="flex items-center gap-2"><Clock className="w-4 h-4" /><span>{experience.durationMinutes} min</span></div>
                      <div className="flex items-center gap-2"><Users className="w-4 h-4" /><span>{experience.minGroupSize}-{experience.maxGroupSize} {language === 'es' ? 'personas' : 'henkilöä'}</span></div>
                    </div>
                    {experience.pricing && (
                      <div className="flex items-center justify-between pt-4 border-t border-slate/10">
                        <span className="text-2xl font-bold text-aurora">{formatCurrency(experience.pricing.basePriceEur)}</span>
                        <span className="text-xs text-slate">{language === 'es' ? 'por persona' : 'per henkilö'}</span>
                      </div>
                    )}
                  </div>
                </div>
