    ...
    <div className="container-narrow py-12">
      <div className="grid gap-10 lg:grid-cols-[1fr_380px]">
        <div>
          ...
          {c.features.length > 0 && <div className="mt-7 flex flex-wrap gap-2">{c.features.map((feature) => <span key={feature} className="rounded-full bg-brand-50 px-3 py-1.5 text-sm text-brand-800">{feature}</span>)}</div>}
        ...
        <aside className="h-fit rounded-2xl border bg-white p-6 shadow-card lg:sticky lg:top-28">
          ...
          {c.bookingUrl ? <a href={c.bookingUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-brand-900 px-5 py-3.5 text-sm font-semibold text-white hover:bg-brand-800">{locale === "fi" ? "Tarkista saatavuus" : locale === "es" ? "Ver disponibilidad" : "Check availability"}<ArrowUpRight className="h-4 w-4" /></a> : null}
          <Link href={"/" + locale + "/contact"} className="mt-3 inline-flex w-full justify-center rounded-full border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">{locale === "fi" ? "Kysy tästä kohteesta" : locale === "es" ? "Preguntar por este lugar" : "Ask about this stay"}</Link>
