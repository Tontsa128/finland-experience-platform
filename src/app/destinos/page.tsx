...
const fallbackContent: Record<string, { es: [string, string]; fi: [string, string] }> = {
  inari: { es: ['Inari', 'La Laponia más auténtica: lagos, naturaleza ártica y cultura sami.'], fi: ['Inari', 'Aitoa Lappia: järviä, arktista luontoa ja saamelaiskulttuuria.'] },
  tampere: { es: ['Tampere', 'Ciudad de lagos, saunas y gastronomía finlandesa en el corazón del país.'], fi: ['Tampere', 'Järvien, saunojen ja suomalaisen gastronomian kaupunki.'] },
  helsinki: { es: ['Helsinki', 'Diseño nórdico, mar, arquitectura y la mejor puerta de entrada a Finlandia.'], fi: ['Helsinki', 'Pohjoismaista designia, merta, arkkitehtuuria ja kaupunkikulttuuria.'] },
  turku: { es: ['Turku', 'Historia, archipiélago y experiencias gastronómicas en la costa occidental.'], fi: ['Turku', 'Historiaa, saaristoa ja elämyksellistä ruokakulttuuria länsirannikolla.'] },
  lapland: { es: ['Laponia', 'Un territorio ártico de nieve, bosques, auroras y aventuras inolvidables.'], fi: ['Lappi', 'Arktinen maailma täynnä lunta, metsiä, revontulia ja seikkailuja.'] },
  lakeland: { es: ['Región de los Lagos', 'Miles de lagos, bosques tranquilos y el auténtico ritmo de la naturaleza finlandesa.'], fi: ['Järvi-Suomi', 'Tuhansia järviä, metsiä ja suomalaisen luonnon rauhallinen rytmi.'] },
};
  ...
  useEffect(() => {
    ...
    DestinationService.getDestinations(true)
      .then((items) => {
        ...
        try {
          managed = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
        } catch {
          managed = [];
        }

        const managedCards: DestinationCard[] = managed.map((item) => ({
          id: item.id,
          slug: item.slug,
          region: item.region,
          ...
          nameEs: item.nameEs,
          nameFi: item.nameFi,
          shortDescriptionEs: item.descriptionEs,
          shortDescriptionFi: item.descriptionFi,
          experienceCount: 0,
        }));
        ...
        <div className="container-site relative py-16 sm:py-20">
          <div className="max-w-3xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-white/80">
              <Snowflake className="h-4 w-4 text-emerald-300" /> Finlandia · viajes seleccionados
            </div>
            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">{language === 'es' ? 'Descubre Finlandia' : 'Tutustu Suomeen'}</h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <div className="flex w-full max-w-md items-center gap-3 rounded-xl border border-white/10 bg-white px-4 py-3 text-slate-900 shadow-xl">
                ...
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={language === 'es' ? 'Buscar destino...' : 'Hae kohdetta...'} className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400" />
              </div>
              <button type="button" onClick={() => setLanguage(language === 'es' ? 'fi' : 'es')} className="rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-white hover:bg-white/15">
      ...
      <section className="container-site py-12 sm:py-16">
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-midnight sm:text-4xl">{language === 'es' ? 'Elige tu Finlandia' : 'Valitse oma Suomesi'}</h2>
          </div>
              ...
              <Link key={destination.id} href={`/destinos/${destination.slug}`} className="group block h-full">
                <article className="h-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-2xl">
                  <div className="relative h-60 overflow-hidden bg-slate-200">
                    <img src={destination.heroImageUrl} alt={title} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                    ...
                    <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-midnight backdrop-blur">
                      <MapPin className="h-3.5 w-3.5 text-emerald-600" /> {destination.region}
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                      <span className="text-sm font-semibold text-slate-500">{destination.experienceCount} {language === 'es' ? 'experiencias' : 'kokemusta'}</span>
                      <span className="inline-flex items-center gap-1 text-sm font-bold text-emerald-700">{language === 'es' ? 'Explorar' : 'Tutustu'} <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
                    </div>
                  </div>
                </article>
              </Link>
